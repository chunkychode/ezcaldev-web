...
export const createCalOwner = async ({ authToken }) => {
  try {
    ...
    const url = `${process.env.NEXT_PUBLIC_CAL_GQL_URL}/calowner?code=${process.env.CAL_GQL_API_KEY}`;
    const post = {
      method: "post",
      headers: {
...
export const createClaimsForCal = async ({
  ...
  try {
    const url = `${process.env.NEXT_PUBLIC_AUTH_URL}/Claims`;
    const claims = [
...
export const setCallbackUrl = async ({ email, callBackUrl, authToken }) => {
  try {
    const url = `${process.env.NEXT_PUBLIC_AUTH_URL}/Claims`;
    const claims = [{ name: "chunky.app:cal:callbackUrl", value: callBackUrl }];
