...
export const registerUser = async ({ email, password }) => {
  try {
    ...
    const url = `${process.env.NEXT_PUBLIC_AUTH_URL}/Register`;
    const body = { email, password };
    const { authToken } = authResp;
...
export const resetPassword = async ({ host, email }) => {
  try {
    ...
    const url = `${process.env.NEXT_PUBLIC_AUTH_URL}/ResetPassword`;
    const body = {
      email,
      returnurl: `${http}://${host}/${process.env.RESET_PASSWORD_TO_PATH}`,
    };
    const post = {
      method: "post",
      body: JSON.stringify(body),
